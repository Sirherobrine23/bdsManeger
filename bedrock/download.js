module.exports = (Vdown, config) => {
    ...
    fetch("https://raw.githubusercontent.com/Bds-Maneger/Raw_files/main/Server.json").then(response => response.json()).then(versions => {
        ...
        if (mine === 'java'){
            ...
            var server_DIR = bds.bds_dir_java;
        }else{
        ...
        downloadBDSchild.on("exit", function (code) {
            if (code === 0) {
                ...
                if (mine === 'java'){
                    ...
                    shelljs.cp("-fv", path.join(bds.tmp_dir, mine_name), path.join(bds.bds_dir_java, "server.jar"))
                } else {
                    var AdmZip = require("adm-zip");
                    const fs = require("fs")
                    if (fs.existsSync(`${server_DIR}/server.properties`)){var OLD_ = true;var old1 = fs.readFileSync(`${server_DIR}/server.properties`, "utf-8");}
                    ...
                    // Unzip 
                    if (OLD_){fs.writeFileSync(`${server_DIR}/server.properties`, old1);}
