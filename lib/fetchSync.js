...
function FetchSync(
    ...
    if (CommandsAvaible.node) {
        const NdFetch = child_process.execFileSync("node", [resolve(__dirname, "fetch/fetch.js")], {env: {...process.env, URL_REQUEST: url, TYPE_REQUEST: binary, REQUEST_OPTIONS: JSON.stringify(options)}})
        if (`${binary}` === "true") {
            const slpath = NdFetch.toString().split("***tmpbdspath\n").join("\n").split("\n").filter(a=>a);const pathFile = slpath[slpath.length - 1];
            const buffer = Buffer.from(readFileSync(pathFile));rmSync(pathFile)
            return {
                Buffer: buffer,
                Path: pathFile,
                save: function(path = "/tmp/aaa"){
        ...
        // Exec
        var Exec = child_process.execSync(`${CommandExec} ${argv.join(" ")}`);
        if (binary) {
