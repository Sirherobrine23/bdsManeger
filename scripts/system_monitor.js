...
// -----------------------------------
const { cpu, mem, currentLoad, processes } = require("systeminformation");


function init_1(){
    cpu().then(data => module.exports.cpu_speed = Math.trunc(data.speed))
    mem().then(data => {
        module.exports.ram_free = Math.trunc(data.free / 1000 / 1000 / 1000)
        module.exports.ram_total = Math.trunc(data.total / 1000 / 1000 / 1000)
    })
    currentLoad().then(data => module.exports.current_cpu = Math.trunc(data.currentLoad))
}
