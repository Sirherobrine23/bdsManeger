...
RUN apt update && \
apt install -y g++ build-essential make dpkg-dev && \
npm install -g nexe
RUN echo 'console.log(process)' > /tmp/test.js && nexe /tmp/build.js --build --output /tmp/test.bin && /tmp/test.bin && em /tmp/test.js
COPY ./ /tmp/core
WORKDIR /tmp/core
RUN npm install
...
RUN apt update && \
apt install -y openjdk-11-jdk openjdk-11-jre screen nginx python make build-essential procps && \
